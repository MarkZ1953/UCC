...
class TablaInformacion(QTableWidget):
    def __init__(self):
        ...
        cabezeras = ["Id", "Nombre", "Fecha", "Magnitud", "Departamento", "No.Muertos Aprox", "Ayuda Estado"]
    ...
    def actualizarTablaInformacion(self):
        ...
        for fila, (idTerremoto, nombreTerremoto, fechaTerremoto, magnitudTerremoto, departamentoTerremoto,
                   noMuertosTerremoto, ayudaEstado) in enumerate(datos):
            ...
            self.setItem(fila, 6, QTableWidgetItem(ayudaEstado))
            self.item(fila, 6).setFlags(~Qt.ItemFlag.ItemIsEditable)
            self.item(fila, 6).setTextAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def celdaSeleccionada(self, row, column):
        ...
        ayudaEstadoTerremoto = self.item(row, 6)
        ...
        if not self.detalleTerremoto.isVisible():
            ...
            self.detalleTerremoto.txtAyudaEstadoTerremoto.setText(ayudaEstadoTerremoto.text())
